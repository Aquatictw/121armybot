    ...
    for user_id, user_info in users_data.items():
        ...
        for card in user_info.get("inventory", []):
            ...
            for row in sheet_rows:
                if row[1] == card_name: # Match by name
                    # Update corp, desc, img, movies
                    updated_corp = row[4]
                    updated_desc = row[2]
                    updated_img = row[3]
                    updated_movies = ast.literal_eval(row[5]) # Convert string representation of list to actual list

                    # Reconstruct the card with updated info, keeping the original tier and count
                    updated_card = (updated_corp, card_name, updated_desc, updated_img, updated_movies, card[5], card[6])
                    updated_inventory.append(updated_card)
                    found_in_sheet = True
                    break
            
            if not found_in_sheet:
                print(f"Warning: Card '{card_name}' (Tier: {card_tier_text}) for user {user_id} not found in Google Sheet. Keeping original data.")
